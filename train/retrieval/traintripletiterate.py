...
parser.add_argument('--valdata_dir',default='/home/shibaorong/modelTorch/out/eyetest.json',
                    help='destination where trained network should be saved')
parser.add_argument('--train_dir',default='/mnt/sdb/shibaorong/logs/eye/both/m1/parameter_61.pkl',
                    help='destination where trained network should be saved')
parser.add_argument('--finetune_dir',default='/mnt/sdb/shibaorong/logs/eye/cls/parameter_13.pkl',
                    help='destination where trained network should be saved')
...
def traintriandcls(mymodel,epoch,cuda_gpu,optimizer,mytraindata,scheduler):
    ...
    mytrainloader = torch.utils.data.DataLoader(mytraindata, batch_size=args.batch_size, shuffle=True,num_workers=15)
    ...
    for index, ((img1,label1), (img2,label2), (img3,label3)) in enumerate(mytrainloader):
        ...
        try:
            ...
            if args.balancedgpu:
                ...
                c1loss = DataParallelCriterion(nn.CrossEntropyLoss())
            else:
            ...
            loss = tripletloss+0.1*(loss1+loss2+loss3)/3
...
def main():
    ...
    for epoch in range(startepoch, args.maxepoch):
        ...
        '''if  (epoch+1)%5==0:
            ...
            testmodel(testargs, cuda_gpu,type='classification')'''
