...
from datasets.siameseData import SiameseIDData
import argparse
import time
...
parser.add_argument('--data_dir',default='/home/shibaorong/modelTorch/out/paristrainpair.json',
                    help='destination where trained network should be saved')
parser.add_argument('--train_dir',default='/mnt/sdb/shibaorong/logs/paris/UWCS/parameter_130.pkl',
                    help='destination where trained network should be saved')
...
def trainUWCS(mymodel,epoch,cuda_gpu,optimizer,mytrainloader,scheduer,clusterinfo):
    global min_loss
    print('epoch {}'.format(epoch + 1))

    trainloss=0.
    record=0
    uwcsloss=buildLoss.UWCSLoss(clusterinfo)
    mymodel.train()
    for index, (img1,label1,id1,img2,label2,id2) in enumerate(mytrainloader):
      ...
      uloss=uwcsloss(f1,id1,f2,id2,target)
...
def main():
    ...
    for epoch in range(startepoch, args.maxepoch):
        ...
        kms=kmeansCluster(args.jsonfile,args.classnum+1)

        clusterinfo=kms.clustering(mymodel,outdim=OUTPUT_DIM[args.backbone])

        mytraindata =SiameseIDData(path=args.data_dir, autoaugment=args.autoaugment)
        mytrainloader = torch.utils.data.DataLoader(mytraindata, batch_size=args.batch_size, shuffle=True)
        trainUWCS(mymodel,epoch,cuda_gpu,optimizer,mytrainloader,scheduler,clusterinfo)
