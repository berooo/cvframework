...
from testt.testcontrasive import parser as partest
from testt.testcontrasive import testGEM,testmodel
...
parser.add_argument('--data_dir',default='/home/shibaorong/modelTorch/out/paris6ktrain.json',
                    help='destination where trained network should be saved')
parser.add_argument('--train_dir',default='/mnt/sdb/shibaorong/logs/paris/retrieval/ResNet/gem_weighted/parameter_09.pkl',
                    help='destination where trained network should be saved')
...
def trainclassification(mymodel,epoch,cuda_gpu,optimizer,mytrainloader,thisloss,scheduler):
    global min_loss
...
def main():
    ...
    '''if os.path.exists(args.finetune_dir):
        ...
        classmodel=builGraph.getModel(args.backbone, args.classnum, args.gpu,
                                 'classification', cuda_gpu=cuda_gpu,pretrained=True)
        classmodel.load_state_dict(checkpoint['model_state_dict'])
        kargs = {}
        kargs['name'] = args.backbone
        kargs['num_classes'] = args.classnum
        mymodel=builGraph.retrievalNet(classmodel.module, finetune=True,**kargs)
        if cuda_gpu:
            mymodel = torch.nn.DataParallel(mymodel, device_ids=args.gpu).cuda()
        for para in mymodel.module._baselayer.parameters():
            para.requires_grad = False
        #optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, mymodel.parameters()))
        ...
        testargs.train_dir = args.finetune_dir
        testOnlinepair(testargs,cuda_gpu)
        testmodel(mymodel,args,cuda_gpu)
'''
    ...
    for epoch in range(startepoch, args.maxepoch):
        ...
        if epoch!=startepoch:
            ...
            testGEM(testargs, cuda_gpu)
        trainclassification(mymodel,epoch,cuda_gpu,optimizer,mytrainloader,thisloss,scheduler)
