...
from datasets.commonDataset import FolderDataset
...
def traintricls(*params):
    global step
    mymodel,epoch,optimizer,mytrainloader,loss_func=params
    global min_loss
    ...
    since = time.time()
    for index, (img,label) in enumerate(mytrainloader):
      train_acc = 0.
...
def main(cfg):
    ...
    dataset=FolderDataset(cfg.INPUT.DATAPATH,mode='train')
    mytrainloader=DataLoader(dataset,batch_size=cfg.TRAIN.BATCH_SIZE,shuffle=False)
    ...
    for epoch in range(startepoch, cfg.OPTIM.MAX_EPOCH):
        ...
        traintricls(mymodel,epoch,optimizer,mytrainloader,loss)
