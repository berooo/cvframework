...
parser.add_argument('--autoaugment',default=True,
                    help='destination where trained network should be saved')
parser.add_argument('--finetune_dir',default='/mnt/sdb/shibaorong/logs/paris/base/trial/parameter_08.pkl',
                    help='destination where trained network should be saved')
...
def trainSiamese(mymodel,epoch,cuda_gpu,optimizer,mytrainloader,scheduer):
    ...
    bar = Bar('[{}]{}'.format('base-DIGIX', 'train'), max=len(mytrainloader))
    since = time.time()
