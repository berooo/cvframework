...
def mac(x,p=1):
  return F.max_pool2d(x,(int(x.size(-2)/p), int(x.size(-1)/p)))
  # return F.adaptive_max_pool2d(x, (1,1)) # alternative


def spoc(x,p=1):
  return F.avg_pool2d(x,(int(x.size(-2)/p), int(x.size(-1)/p)))
  # return F.adaptive_avg_pool2d(x, (1,1)) # alternative


def gem(x, p=4, eps=1e-6):
...
def ApplyPcaAndWhitening(data,
  ...
  '''
  pca_mean = torch.from_numpy(pca_mean).cuda().float()
  pca_vars = torch.from_numpy(pca_vars).cuda().float()
  pca_matrix = torch.from_numpy(pca_matrix).cuda().float()
