...
class LandmarkUpsample(nn.Module):
  def __init__(self,numkeypoints,in_channel=256):
    ...
    self.conv10=nn.Conv2d(16,numkeypoints,1,1,0)
    self.relu=nn.ReLU(inplace=True)
...
class Network(nn.Module):
  def __init__(self,pretrained=False,**kwargs):
    ...
    self.feature_extractor=VGG16Extractor(pretrained)
    self.upsampling=LandmarkUpsample(kwargs['numkeypoints'],512)
